    ...
    def _start_download(self, url, path, callback, errback):
        try:
            ...
            with closing(urllib.request.urlopen(url)) as sock:
                fullname = path(sock, url, overwrite=True)  # RTS, make sure to overwrite the original filename

                if not os.path.exists(fullname): # RTS - Added check to see if file is already downloaded
                    ...
                    with open(fullname, 'wb') as fd:
                        while True:
                            ...
                            if not rec:
                                ...
                                fd.write(rec)
                else:   # RTS - Clean up properly if the file is already downloaded
                    print("Ignoring previously downloaded file: " + fullname)
                    # with self.mutex:
                    #     self._close(callback, [{'path': fullname}], server)
